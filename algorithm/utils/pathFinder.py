        ...
        for op in self.get_visit_options(len(all_view_positions)):
            ...
            for c in combination: # run the algo some times ->
                ...
                for i in range(len(_permutation) - 1):
                    ...
                    if to_item.screenshot_id != -1:
                        optimal_obstacle_order.append(to_item.screenshot_id)
               

            if optimal_path:
                # print(f"Found optimal path: {optimal_path}")
                # print("Found optimal obstacle order: ", optimal_obstacle_order)
                # if found optimal path, return
                break

        return optimal_path, distance, optimal_obstacle_order
