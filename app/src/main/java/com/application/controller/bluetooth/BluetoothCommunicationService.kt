 ...
 */
class BluetoothCommunicationService(private val mHandler: Handler) {
    // Member fields
    ...
    private fun updateUserInterfaceTitle() {
        ...
        Log.d(TAG, "updateUserInterfaceTitle() " + mNewState + " -> " + state_current)
        mNewState = state_current
    ...
    fun connect(device: BluetoothDevice, secure: Boolean) {
        ...
        // Cancel any thread attempting to make a connection
        if (state_current == STATE_CONNECTING) {
            if (mConnectThread != null) {
                mConnectThread!!.cancel()
                mConnectThread = null
            }
    ...
    fun write(out: ByteArray?) {
        ...
        synchronized(this) {
            if (state_current != STATE_CONNECTED) return
            r = mConnectedThread
        }
    ...
    @SuppressLint("MissingPermission")
     private inner class AcceptThread(secure: Boolean) : Thread() {
        ...
        override fun run() {
            ...
            while (state_current != STATE_CONNECTED) {
                try {
                ...
                if (socket != null) {
                    synchronized(this@BluetoothCommunicationService) {
                        when (state_current) {
    ...
    companion object {
        // Debugging
        private const val TAG = "BluetoothCommunicationService"

        // Name for the SDP record when creating server socket
        private const val NAME_SECURE = "BluetoothCommunicationSecure"
        private const val NAME_INSECURE = "BluetoothCommunicationInsecure"
